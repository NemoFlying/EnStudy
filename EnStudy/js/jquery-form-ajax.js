(function ($) {
    ...
    function ProgressHandler(fileHandler) {
        var progressContainer = $("div.nemo-progress-group div.progress-group");//查看是否已经产生了进度条container;
        if (progressContainer.length == 0) {
            //创建进度条Container
            $("body").append(`
                <div class="nemo-progress-group" style=" z-index: 9999;;position:fixed;top:0px;bottom:0px;right:0px;left:0px;background-color: cadetblue;padding:10%">
                    <div class="progress-group">
                        <div class=" progress-demo " style=" width: 100%; display:none" >
                            <div class="row">
                                <div class="col-sm-2"></div>
                                <div class="col-sm-5"><h4>Demo</h4></div>
                            </div>
                            <div class=" row ">
                                <div class="col-sm-2"></div>
                                <div class="col-sm-5">
                                ...
                                <div class="col-sm-1">
                                    <span class="glyphicon glyphicon-ok" style="color:white; visibility: hidden;"></span>
                                </div>
                                <div class="col-sm-2">
                    ...
                    <div class="progress-controll">
                        <div class="">
                            <button class="closeprogress btn btn-success ">关闭</button>
                            <button class="cancelprogress btn  btn-danger">取消上传</button>
                        </div>
                    </div>
                </div>
            `);
            progressContainer = $("div.nemo-progress-group div.progress-group");
            //绑定事件
            $("div.nemo-progress-group div.progress-controll button.closeprogress").click(function () {
                //关闭上传框
                $("div.nemo-progress-group").remove();
            }).hide();
        }
        var progress = progressContainer.find("div.progress-demo").clone();
        progress.removeClass("progress-demo").addClass("progress-item").show();
        ...
        fileHandler.on('sendChange', function () {
            switch (fileHandler.SendStatus) {
                case 0:
                    break;
                case 1:   //文件上传完成
                    //$("div.nemo-progress-group div.progress-controll button.closeprogress").show();
                    //判断是否所有文件上传完成
                    progress.find("span.glyphicon-ok").css("visibility", "visible");
                    if (progressContainer.find("div.progress-item:nth(0) span.glyphicon-ok").css("visibility") == "visible") {
                        $("div.nemo-progress-group div.progress-controll button.closeprogress").show().click();
                    }
                    break;
                case -1:  //发生异常
                    console.log(fileHandler.ErrMsg);
                    break;
            }
            ...
            $.ajax({
                ...
                success: function (reData) {
                    ...
                    if (reData.BlockSaveStatus) {
                        ...
                        self.SendStatus = -1;
                        self.ErrMsg = reData.FileSaveErrMsg;
                        if (typeof (defaults.error) == "funciton") {
                            defaults.error(fileHandler.ErrMsg);
                        }
                    }
                },
                error: function (e) {
